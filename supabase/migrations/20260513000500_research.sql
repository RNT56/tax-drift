...
alter table research_subjects enable row level security;
...
alter table research_copilot_threads enable row level security;
alter table research_copilot_messages enable row level security;

create policy research_subjects_owner_all
...
create policy research_source_snapshots_owner_all
  on research_source_snapshots
  for all
  using (user_id = auth.uid()::text)
  with check (user_id = auth.uid()::text);

create policy research_evidence_owner_all
  on research_evidence
  for all
  using (user_id = auth.uid()::text)
  with check (user_id = auth.uid()::text);

create policy research_metrics_owner_all
  on research_metrics
  for all
  using (user_id = auth.uid()::text)
  with check (user_id = auth.uid()::text);

create policy research_events_owner_all
  on research_events
  for all
  using (user_id = auth.uid()::text)
  with check (user_id = auth.uid()::text);

create policy research_copilot_threads_owner_all
  on research_copilot_threads
  for all
  using (user_id = auth.uid()::text)
  with check (user_id = auth.uid()::text);

create policy research_copilot_messages_owner_all
  on research_copilot_messages
  for all
  using (user_id = auth.uid()::text)
  with check (user_id = auth.uid()::text);
